...
void showStudent(SV a[], int n) {
   printLine(100);
    cout <<"\nID\tHo va ten\t\tGioi tinh\tTuoi\tToan\tLy\tHoa\tDiemTB\tHoc luc";
    for(int i = 0; i < n; i++) {
...
void sapXepTheoTen(SV a[], int n) {
    ...
    char tenSV2[30];
    for(int i = 0;i < n; i++) {
        strcpy(tenSV1, tachTen(a[i]));
        for(int j = i+1; j < n; j++) {
            strcpy(tenSV2, tachTen(a[j]));
...
void ghiFile(SV a[], int n, char fileName[]) {
    ...
    fp = fopen (fileName, "w");
    for(int i = 0;i < n;i++){
        fprintf(fp, "%-5d %-30s %-5s %-5d %-10.2f %-10.2f %-10.2f %-10.2f %-10s\n", a[i].id, a[i].ten, a[i].gioiTinh, 
